...
export default function DashboardClient({ subdomains }: { subdomains: Subdomain[] }) {
  ...
  async function handleSaveEdit(sub: Subdomain) {
    ...
    try {
      ...
      if (editTarget && editTarget !== sub.target) body.customTarget = editTarget;
      if ((sub.type === "vercel" || sub.type === "netlify") && editTxtValue && editTxtValue !== sub.vercelTxtValue) {
        body.vercelTxtValue = editTxtValue;
      }
        ...
        setList((prev) =>
          prev.map((s) =>
            s.id === sub.id
              ? {
                  ...s,
                  target: body.customTarget ?? s.target,
                  vercelTxtValue: body.vercelTxtValue ?? s.vercelTxtValue,
                }
              : s
          )
        );
      ...
      {list.map((sub) => {
        ...
        return (
          ...
          >
            ...
            {/* Edit panel for custom-target subdomains */}
            {isEditing && (
                ...
                <div className="pt-4 flex flex-col gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-medium mb-1.5 text-zinc-500">
                      {sub.type === "vercel" ? "CNAME value" : "Netlify target"}
                    </label>
                    <input
                      type="text"
                      value={editTarget}
                  ...
                  {(sub.type === "vercel" || sub.type === "netlify") && (
                    <div>
                      <label className="block text-xs font-medium mb-1.5 text-zinc-500">
                        {sub.type === "vercel" ? "Vercel TXT verification value" : "Netlify TXT verification value"}
                      </label>
                      <input
                        type="text"
                        value={editTxtValue}
                      ...
                      <p className="text-xs text-zinc-400 mt-1">
                        {sub.type === "vercel"
                          ? "Copy the TXT value shown by Vercel for _vercel."
                          : "Paste the TXT value shown by Netlify. from-mm.dev uses subdomain-owner-verification as the record name."}
                      </p>
                    </div>
                  )}
              ...
              <div className="px-5 pb-5 pt-0 border-t border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950">
                <div className="pt-4 flex flex-col gap-3">
                  ...
                  {sub.type === "github_pages" ? (
                    ...
                    <ol className="text-sm text-zinc-600 dark:text-zinc-400 list-decimal list-inside flex flex-col gap-1.5">
                      <li>Go to your Netlify site <strong>Site configuration → Domain management</strong></li>
                      <li>Click <strong>Add a domain</strong> and enter <code className="bg-zinc-100 dark:bg-zinc-800 px-1 rounded">{sub.subdomain}.{BASE_DOMAIN}</code></li>
                      <li>Netlify shows a target hostname - that value is already set in your DNS</li>
                      <li>If the site target changes, click <strong>Edit</strong> above and update it</li>
                      <li>HTTPS is automatic once Netlify validates the domain</li>
                    </ol>
                  )}
